...
contract WaitAndWin is ERC20("Wait And Win", "WAW"), Owned {
        ...
        payable
    {
        ...
        //== first, get the magic number generated by the contract for each user
        /* uint256[] memory initialTab;
        for (uint256 i = 0; i < vPools[_poolID].numberOfVoters; i++) {
            initialTab[i] = i + 1;
        }

        uint256 userMagic;
        uint256[] memory usersMagic;
        usersMagic = rankDistribution(_poolID, initialTab);

        // get the magic number for the current user (msg.sender)
        for (uint256 i = 0; i < vPools[_poolID].numberOfVoters; i++) {
            if (msg.sender == vPools[_poolID].voterAddresses[i]) {
                userMagic = usersMagic[i];
            }
        }

        //== second, calculate the winning votes
        uint256[] memory _winningVotes;
        _winningVotes = winningVotes(_poolID);

        // check if the user rank is eequal to one of the winning votes
        bool isWinningVote;
        isWinningVote = ElementOf(userMagic, _winningVotes);

        require(isWinningVote, "You didn't win");

        // transfer the prize to the winner
        // the pize must be distributed equally to all winners
        uint256 numberOfWinners;
        numberOfWinners = _winningVotes.length;
        ...
        //uint256 prize = vPools[_poolID].prize;
